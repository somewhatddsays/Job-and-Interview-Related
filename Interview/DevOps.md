...
1. **Host Volume &mdash;** Maps a directory from the host machine to the container.
2. **Anonymous Volume &mdash;** Docker manages the volumes without a specific name.
3. **Named Volume &mdash;** User-defined and persisted independently of containers.
...
#### If you want to add an existing resources to the Terraform state file?
**Use the command**:
...
In Kubernetes, all pods can communicate with each other by default within the
cluster because the cluster network is **flat and non-restrictive** unless **Network
Policies** restrict it.

#### What are Helm charts?
A Helm chart is a package of YAML templates used to deploy Kubernetes application through versioned, repeatable, and configurable deployments.
...
Infrastructure is usually managed by the **DevOps Team** using laC tools like **Terraform**, **CloudFormation**, and **Ansible**.
...
Most common: **Gitflow**
- **main/ master &rarr;** Stable production
- **develop &rarr;** Active development
- **feature &rarr;** New features
- **release &rarr;** Pre-production
- **hotfix &rarr;** Quick patches on production
...
#### What plugin do you use in Jenkins?
**Common ones**:
* Git
* Pipeline
* Credentials
...
* EC2 AMI rollback
* Terraform rollback (manual state revert)
* Git Revert
...
* **Merge**: Combines changes with merge commit.
...
* git fetch **&rarr;** downloads but doesn't merge
* git pull **&rarr;** downloads + merges automatically
...
#### Docker stops & restarts &mdash; data lost. How to fix?
...
**CMD**: Default command (can be overridden) (unless using --entrypoint).
